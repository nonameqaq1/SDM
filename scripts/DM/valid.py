...
if __name__ == "__main__":
    ...
    parser.add_argument("--flowae_checkpoint",
                        default="/home/logs_training/AE/smmnist/smmnist_test/snapshots/RegionMM.pth")
    parser.add_argument("--checkpoint",
                        default="/home/logs_training/DM/smmnist/smmnist_test/snapshots/flowdiff.pth")
    ...
    config["flow_params"]["model_params"]["generator_params"]["pixelwise_flow_predictor_params"][
        "estimate_occlusion_map"] = args.estimate_occlusion_map
    ...
    for i_iter, batch in enumerate(valid_dataloader):
        ...
        for i_autoreg in range(NUM_AUTOREG):
            i_pred_video = model.sample_one_video(cond_scale=1.0, real_vid=i_real_vids.cuda())[
    ...
    for i_bs in tqdm(range(len(result_videos))):
        ...
        select_scores.append([np.abs(origin_feats[i_bs] - result_feats[i_bs * args.num_sample_video + i]).sum() for i in
                              range(args.num_sample_video)])

    fvd = []

    selected_index = np.argmin(np.array(select_scores), axis=-1)
    best_videos = torch.from_numpy(
        np.array([result_videos[i, selected_index[i]] for i in range(selected_index.shape[0])]))
