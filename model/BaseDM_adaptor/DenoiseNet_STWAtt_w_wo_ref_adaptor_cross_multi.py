...
class Unet3D(nn.Module):
    ...
    def forward(self, x, time, cond_frames, point,cond_fea=None, cond=None, null_cond_prob=0., none_cond_mask=None):
        ...
        x = torch.cat([cond_frames[:,:,:-1], x], dim=2)
        assert tc == self.tc
        assert tp == self.tp
        assert cond_fea.shape[2] == tc-1+tp
        time_rel_pos_bias   = self.time_rel_pos_bias(tc-1+tp, device=x.device)
        # b c t h w
        ...
        h = []
        for i,(block1, STW_attn1 ,block2, STW_attn2, adaptor, downsample) in enumerate(self.downs):
            if i == 0:
                x = block1(x, t)
                x = block2(x, t)
        ...
        x_fin = self.final_conv(x)[:,:,tc-1:]
        x_occ = self.occlusion_map(x)[:,:,tc-1:]
