...
class FlowDiffusion(nn.Module):
    def __init__(self, 
        ...
        self.unet = Unet3D(
            ...
            cond_num=dataset_params['train_params']['cond_frames'],#tm
            pred_num=dataset_params['train_params']['pred_frames'],#tp
    ...
    def forward(self, real_vid):
        ...
        b, _, nf, H, W = real_vid.size()#nf = 15
        ret = {}
        ...
        with torch.no_grad():
            ...
            point = source_region_params['shift'].detach().cuda()
            for idx in range(nf):
        ...
        if self.is_train:
            ...
            #在self.diffusion的forward中输入参考prompt中心点
            loss, pred = self.diffusion.forward(frames[:,:,:self.cond_frame_num], frames[:,:,self.cond_frame_num:self.cond_frame_num+self.pred_frame_num], point=point,cond_fea = ref_img_fea)
            ret['loss'] = loss
    ...
    def sample_one_video(self, cond_scale, real_vid):
        ...
        pred = self.diffusion.sample(x_cond, cond_fea=ref_img_fea, batch_size=1, cond_scale=cond_scale,point=point)
        if self.use_residual_flow:
