...
class FlowDiffusion(nn.Module):
    def __init__(self, 
        ...
        if Unet3D_architecture == "DenoiseNet_STWAtt_w_w_ref_adaptor_cross_multi_traj_u12":
            from model.BaseDM_adaptor.DenoiseNet_STWAtt_w_w_ref_adaptor_cross_multi_traj_u12 import Unet3D #bair
            ...
            from model.BaseDM_adaptor.DenoiseNet_STWAtt_w_w_ref_adaptor_cross_multi_traj_ada import Unet3D #kth
        else:
            NotImplementedError()
    ...
    def forward(self, real_vid):
        ...
        if self.is_train:
            ...
            loss, pred = self.diffusion(frames[:,:,:self.cond_frame_num], frames[:,:,self.cond_frame_num:self.cond_frame_num+self.pred_frame_num], point=point,cond_fea = ref_img_fea)
            ret['loss'] = loss
