...
class RegionPredictor(nn.Module):
    ...
    def region2affine(self, region):
        ...
        assert mean is not None, "point 不能为 None"
        region_params = {'shift': mean}#区域中心位置，point
    ...
    def forward(self, x):
        ...
        feature_map = self.predictor(x)#shape:B,35,32,32
        # 生成特征图
        prediction = self.regions(feature_map)#shape:B,10,26,26
